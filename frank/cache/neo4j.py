...
def get_driver():
  ...
  driver = GraphDatabase.driver(uri, auth=(config.config['neo4j_username'],config.config['neo4j_password']), encrypted=False)
  return driver
      ...
      if node[2]:       
        for rel in node[2]:
          if rel.start_node._properties['id'] not in alists:
            ...
            alists[rel.start_node._properties['id']] = {"id": id , "label": label, "ntype": nodeType, "nstate": nodeState , "h":op}
          if rel.end_node._properties['id'] not in alists:
            ...
            if len(id) > 5:
              label = f"{id[0:2]}..{id[-3]}{id[-2]}{id[-1]}"
            alists[rel.end_node._properties['id']] = {"id": id , "label": label, "ntype": nodeType, "nstate": nodeState, "h":op}
...
def _get_node(tx, sessionId, alistNodeId):
    alist = {}
    results = tx.run(f"MATCH (n:Alist {{sessionId:'{sessionId}', id:'{alistNodeId}' }}) RETURN n")
    for node in results:
      if node[0]:
...
def _search_cache(tx, alist_to_instantiate:Alist, attribute_to_instantiate, search_attributes)->(bool, list):
    ...
    conditions = " AND ".join([f"n.{x}='{alist_to_instantiate.instantiation_value(x)}'" for x in search_attributes])    
      ...
      if node[0]:
        try:
          ...
          if alist.is_instantiated(attribute_to_instantiate):
            alist.set(attribute_to_instantiate, alist.instantiation_value(attribute_to_instantiate))
...
def create_node(alist, parent=None, edge=None, sessionId='0'):
  ...
  with driver.session() as session:
    node_id = session.write_transaction(_create_update, alist, parent, edge, sessionId)
    return node_id
...
def search_cache(alist_to_instantiate, attribute_to_instantiate, search_attributes=[]):
  ...
  driver = get_driver()
  with driver.session() as session:
    node = session.read_transaction(_search_cache, alist_to_instantiate, attribute_to_instantiate, search_attributes)
    return node
